...
import { saveMessage } from './services/message.service.js';  // ← NEW
...
io.on('connection', socket => {
    ...
    socket.on('project-message', async data => {
        ...
        const sender = data.sender;                         // { _id, email }
        const projectId = socket.roomId;

        // ── 1. Broadcast to other users in the room ──────────────────────────
        socket.broadcast.to(socket.roomId).emit('project-message', data);

        // ── 2. Persist the user's message ────────────────────────────────────
        try {
            await saveMessage({ projectId, sender, message });
        } catch (err) {
        ...
        if (aiIsPresentInMessage) {
            ...
            const aiPayload = { message: result, sender: aiSender };

            // Emit AI reply to everyone in the room
            io.to(socket.roomId).emit('project-message', aiPayload);

            // ── 4. Persist the AI reply ───────────────────────────────────────
            try {
                await saveMessage({ projectId, sender: aiSender, message: result });
            } catch (err) {
